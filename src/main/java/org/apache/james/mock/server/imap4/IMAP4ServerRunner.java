...
public class IMAP4ServerRunner {
    ...
    private IMAPServerFactory createIMAP4ServerFactory(MailboxManager mailboxManager, HierarchicalConfiguration configuration, Logger logger) throws Exception {
        IMAPServerFactory imapServerFactory = new IMAPServerFactory();
        imapServerFactory.setFileSystem(createFileSystem());
        imapServerFactory.setImapDecoder(new DefaultImapDecoderFactory().buildImapDecoder());
        imapServerFactory.setImapEncoder(new DefaultImapEncoderFactory().buildImapEncoder());
        DefaultImapProcessorFactory defaultImapProcessorFactory = new DefaultImapProcessorFactory();
        defaultImapProcessorFactory.setMailboxManager(mailboxManager);
// Mailing list configuration
//        defaultImapProcessorFactory.setMailboxTyper();
//        defaultImapProcessorFactory.setSubscriptionManager();
        imapServerFactory.setImapProcessor(defaultImapProcessorFactory.buildImapProcessor());
    ...
    private void destroyPop3ServerFactory() {
        if (imap4ServerFactory.getServers() != null) {
            try {
                imap4ServerFactory.destroy();
            } catch (Exception e) {
